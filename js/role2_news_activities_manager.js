...
$(function(){
	...
	var datatosend = {};
	var delete_redirect_url = "";
	if (url.indexOf("news") > 0) {
		datatosend={ // type = 3 表示新闻类型的数据
			"type":"3"
		};
		delete_redirect_url = "publish_news.php";
	} else if (url.indexOf("activi") > 0) {
		datatosend={ // type = 2 表示活动类型的数据
			"type":"2"
		};
		delete_redirect_url = "publish_activities.php";
	}
	var type = datatosend.type;
	var article_type = $("#article_type").val();
	$.ajax({
		type:"post",
		url:'action/list_article.php',//处理的后台action的URL
		data:datatosend,
		crossDomain:true,
		dataType:"json",
			...
			if(data.SUCCESS==true){
				...
				for(var i=0;i<data.DATA.length;i++){
					if(data.DATA[i].ISPOST=="1")
						...
						var ISPOST="待审核";
					txt+="<tr class='industry' style='display:none;'><td class='tc'><input name='id[]' type='checkbox'  value='"+data.DATA[i].ID+"'></td><td class='lie_1'> <nobr><b><a href='article.php?id="+data.DATA[i].ID+"&type="+type+"'>"+data.DATA[i].MAINTITLE+"</b></nobr></td> <!--标签ID--><td class='lie_2'><nobr><b>"+data.DATA[i].AUTHOR+"</b></nobr> </td><td><b>"+data.DATA[i].CREATIME+"</b></td> <td><b>"+ISPOST+"</b></td></tr>";
				}
	...
	$(".delete_btn").click(function(){
		$("input:checked").each(function(){
			...
			$.ajax({
				...
				url:'../../backstage/role2/action/delete_article.php',//处理的后台action的URL
		...
		window.location.href=delete_redirect_url;
	});
